...
class MainFrame(wx.Frame):
	def __init__(self, app, session=None):
		...
		icons = wx.IconBundle()
		icons.AddIconFromFile(os.path.join(app.cwd, "images", "write++-16.png"), wx.BITMAP_TYPE_PNG)
		icons.AddIconFromFile(os.path.join(app.cwd, "images", "write++-32.png"), wx.BITMAP_TYPE_PNG)
		self.SetIcons(icons)
	...
	def SaveSession(self, filename=None):
		...
		session.write(ElementTree.tostring(root, encoding="utf-8", xml_declaration=True, pretty_print=True))
		session.close()
